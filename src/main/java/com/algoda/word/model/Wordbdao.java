...
public class Wordbdao extends DBConnPool implements Worddao {
    private final String WORD_INSERT = "insert into  VOCAB(word, MEANINGA, MEANINGB, MEANINGC, SIMILAR_WORDA, SIMILAR_WORDB, SIMILAR_WORDC, day) values (?,?,?,?,?,?,?,?)";
    private final String WORD_UPDATE = "update VOCAB set  MEANINGA=?, MEANINGB=?, MEANINGC=?, SIMILAR_WORDA=?, SIMILAR_WORDB=?, SIMILAR_WORDC=?, day=? where word = ?";
    ...
    public void insertWord(Worddto dto) {
        ...
        try {
            this.psmt = this.con.prepareStatement("insert into  VOCAB(word, MEANINGA, MEANINGB, MEANINGC, SIMILAR_WORDA, SIMILAR_WORDB, SIMILAR_WORDC, day) values (?,?,?,?,?,?,?,?)");
    ...
    public void updateWord(Worddto dto) {
        ...
        try {
            this.psmt = this.con.prepareStatement("update VOCAB set  MEANINGA=?, MEANINGB=?, MEANINGC=?, SIMILAR_WORDA=?, SIMILAR_WORDB=?, SIMILAR_WORDC=?, day=? where word = ?");
    ...
    public void deleteWord(Worddto dto) {
        ...
        try {
            this.psmt = this.con.prepareStatement("delete VOCAB where word=?");
            this.psmt.setString(1, dto.getWord());
            this.psmt.executeUpdate();
        } catch (SQLException var3) {
            throw new RuntimeException(var3);
    ...
    public Worddto getWord(Worddto dto) {
        ...
        try {
            this.psmt = this.con.prepareStatement("select * from VOCAB where word=?");
            this.psmt.setString(1, dto.getWord());
    ...
    public List<Worddto> getWordList(Worddto dto) {
        ...
        try {
            this.psmt = this.con.prepareStatement("select * from VOCAB where day=?");
    ...
    public List<Worddto> selectListPage(Map<String, Object> map) {
        ...
        String query = " SELECT * FROM (     SELECT Tb.*, ROWNUM rNum FROM (         SELECT * FROM vocab ";
    ...
    public List<Worddto> getWordBList() {
        ...
        try {
            this.psmt = this.con.prepareStatement("select * from VOCAB");
            this.rs = this.psmt.executeQuery();
